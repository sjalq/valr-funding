                ...
                , editMsg =
                    \patch ->
                        ...
                        , edited = Dict.singleton index patch
                            ...
                            |> editMsg
                , queryMsg =
                    \query -> queryMsg <| QIndexed index query
                }
                val
...
viewDictEditor { typeDict, editMsg, queryMsg, keyType, valueType } { cursor, items } =
    if List.isEmpty items then
        ...
        let
            ...
            viewPairKey : Int -> Value -> Value -> Html msg
            viewPairKey index key val =
                value
                    { typeDict = typeDict
                    , type_ = keyType
                    , editMsg =
                        \patch ->
                            ...
                            dictPatch
                                |> PDict
                                |> editMsg
                    , queryMsg = \query -> queryMsg <| QIndexed index query
                    }
                    key

            viewPairValue : Int -> Value -> Value -> Html msg
            viewPairValue index key val =
                value
                    { typeDict = typeDict
                    , type_ = valueType
                    , editMsg =
                    ...
                    (List.indexedMap
                        (\rawIndex ( key, val ) ->
                            Html.tr
                                ...
                                , HtmlExtra.td [ viewPairKey (rawIndex + cursor) key val ]
                                , HtmlExtra.td [ Fusion.Colors.purpleText ", " ]
                                , HtmlExtra.td [ viewPairValue (rawIndex + cursor) key val ]
                                , HtmlExtra.td [ Fusion.Colors.purpleText ")" ]
                                ]
                        )
                        values
